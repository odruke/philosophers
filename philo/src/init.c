...
void init_data(t_data *data)
{
    ...
    i = -1;
    data->end_sim = false;
    data->philos = (t_philo *)safe_malloc(sizeof(t_philo) * data->nb_philos, __FILE__, __LINE__);
    data->forks = (t_fork *)safe_malloc(sizeof(t_fork) * data->nb_philos, __FILE__, __LINE__);
    safe_mutex_handle(&data->data_mutex, (t_thrhandle){INIT, __FILE__, __LINE__});
    safe_mutex_handle(&data->print_mutex, (t_thrhandle){INIT, __FILE__, __LINE__});
    while (++i < data->nb_philos)
    {
        safe_mutex_handle(&data->forks[i].fork, (t_thrhandle){INIT, __FILE__, __LINE__});
