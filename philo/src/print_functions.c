...
// 		printf("%-6ld %i has taken a fork.\t\tfirst fork_id = %i\t\tsecond fork_id = %i\n", elapsed_time, philo->id, philo->fork[0]->fork_id, philo->fork[1]->fork_id);
// 	else if (status == LEAVE_FORK && !get_bool(&data->data_mutex, &data->end_sim))
// 		printf("%-6ld %i has left a fork.\t\tfirst fork_id = %i\t\tsecond fork_id = %i\n", elapsed_time, philo->id, philo->fork[0]->fork_id, philo->fork[1]->fork_id);
